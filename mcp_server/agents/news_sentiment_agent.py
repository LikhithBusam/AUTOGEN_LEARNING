...
# Optional import for TextBlob - fallback if not available
try:
    from textblob import TextBlob
...
class NewsSentimentAgent:
    ...
    def __init__(self, model_client: OpenAIChatCompletionClient):
        ...
        self.positive_keywords = [
            ...
            'expansion', 'revenue increase', 'strong', 'positive', 'gain', 'rise',
            'surge', 'breakthrough', 'success', 'record', 'milestone', 'acquire',
            'merger', 'partnership', 'innovation', 'launch', 'approval', 'rally',
            'boom', 'soar', 'excel', 'robust', 'healthy', 'optimistic'
        ]
        ...
        self.negative_keywords = [
            ...
            'debt', 'bankruptcy', 'lawsuit', 'investigation', 'fraud', 'scandal',
            'falling', 'drop', 'plunge', 'crash', 'weak', 'disappointing',
            'miss', 'cut', 'layoffs', 'closure', 'suspend', 'delay', 'volatile',
            'uncertainty', 'concern', 'warning', 'risk', 'trouble', 'struggle'
        ]
    ...
    async def _analyze_articles_sentiment(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for article in articles:
            try:
                ...
                # Combine sentiments with better weighting
                if TEXTBLOB_AVAILABLE and abs(polarity) > 0.1:
                    combined_score = (polarity * 0.6) + (keyword_sentiment * 0.4)
                else:
                    combined_score = keyword_sentiment
                ...
                # Skip problematic articles but log the error
                print(f"Warning: Failed to analyze article sentiment: {e}")
                continue
    ...
    def _analyze_financial_keywords(self, text: str) -> float:
        ...
        # Apply scaling factor to make sentiment more pronounced
    ...
    def _calculate_overall_sentiment(self, sentiment_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Calculate confidence based on consistency
        scores = [analysis.get("sentiment_score", 0) for analysis in sentiment_analyses]
        if len(scores) > 1:
            score_range = max(scores) - min(scores)
            consistency = max(0, 1.0 - (score_range / 2.0))
        else:
            consistency = 1.0
        
        confidence = min(consistency * abs(overall_score) * 2, 1.0)
        
        return {
            "score": round(overall_score, 3),
            "label": label,
            "confidence": round(confidence, 3),
            "positive_articles": len([a for a in sentiment_analyses if a.get("sentiment_label") == "positive"]),
            "negative_articles": len([a for a in sentiment_analyses if a.get("sentiment_label") == "negative"]),
            "neutral_articles": len([a for a in sentiment_analyses if a.get("sentiment_label") == "neutral"])
        }
    ...
    def _extract_themes(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all_text.strip():
                return {"main_topics": [], "financial_themes": [], "word_frequency": {}}
            ...
            stop_words = {
                'the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 'can', 'had', 'her', 'was', 'one', 
                'our', 'out', 'day', 'get', 'has', 'him', 'his', 'how', 'its', 'may', 'new', 'now', 'old', 
                'see', 'two', 'way', 'who', 'boy', 'did', 'man', 'put', 'say', 'she', 'too', 'use', 'will',
                'with', 'from', 'they', 'this', 'that', 'have', 'been', 'said', 'each', 'which', 'their',
                'time', 'year', 'more', 'than', 'also', 'some', 'what', 'make', 'like', 'into', 'over',
                'such', 'only', 'well', 'very', 'most', 'other', 'many', 'much', 'more'
            }
            ...
            theme_keywords = {
                ...
                'growth': ['growth', 'expansion', 'increase', 'rising', 'growing'],
                'merger': ['merger', 'acquisition', 'buyout', 'takeover', 'deal'],
                'regulation': ['regulation', 'policy', 'government', 'regulatory', 'compliance'],
                'technology': ['technology', 'digital', 'innovation', 'tech', 'artificial', 'intelligence'],
                'market': ['market', 'trading', 'stock', 'shares', 'investment']
            }
    ...
    def _calculate_sentiment_trend(self, sentiment_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Sort by publication date (most recent first)
            dated_analyses = [a for a in sentiment_analyses if a.get("published_at")]
            
            if len(dated_analyses) < 2:
                # If no dates, use order as proxy
                sorted_analyses = sentiment_analyses
            else:
                try:
                    sorted_analyses = sorted(
                        dated_analyses, 
            ...
            recent_scores = [a.get("sentiment_score", 0) for a in sorted_analyses[:mid_point]]
            older_scores = [a.get("sentiment_score", 0) for a in sorted_analyses[mid_point:]]
    ...
    def _extract_market_themes(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add market-specific analysis
            market_indicators = {
                'inflation': ['inflation', 'cpi', 'price', 'cost', 'consumer'],
                'interest_rates': ['interest', 'rates', 'fed', 'federal', 'monetary', 'policy'],
                'employment': ['employment', 'jobs', 'unemployment', 'labor', 'hiring'],
                'gdp': ['gdp', 'growth', 'economy', 'economic', 'recession'],
                'trade': ['trade', 'tariff', 'export', 'import', 'international']
            }
            ...
            return {"market_indicators": [], "error": str(e)}
    ...
    def _analyze_sector_sentiment(self, articles: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        try:
            sectors = {
                'technology': ['tech', 'software', 'digital', 'ai', 'cloud', 'data', 'internet'],
                'healthcare': ['health', 'medical', 'pharma', 'drug', 'biotech', 'hospital'],
                'financial': ['bank', 'finance', 'credit', 'loan', 'insurance', 'payment'],
                'energy': ['oil', 'gas', 'energy', 'renewable', 'solar', 'wind', 'electric'],
                'retail': ['retail', 'consumer', 'shopping', 'ecommerce', 'store', 'sales']
            }
            ...
            for sector, keywords in sectors.items():
                ...
                if sector_articles:
                    ...
                    for article in sector_articles:
                        ...
                        sentiment = self._analyze_financial_keywords(text)
                        sentiments.append(sentiment)
                    
                    if sentiments:
                        sector_sentiments[sector] = round(sum(sentiments) / len(sentiments), 3)
    ...
    def _identify_market_concerns(self, articles: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            concern_keywords = [
                'recession', 'inflation', 'crisis', 'volatility', 'uncertainty',
                'debt', 'default', 'risk', 'concern', 'worry', 'fear', 'decline',
                'threat', 'challenge', 'pressure', 'weakness'
            ]
    ...
    def _identify_market_opportunities(self, articles: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            opportunity_keywords = [
                'opportunity', 'growth', 'expansion', 'innovation', 'breakthrough',
                'launch', 'emerging', 'potential', 'upside', 'optimism',
                'recovery', 'improvement', 'momentum', 'strength'
            ]
    ...
    def _create_sentiment_comparison(self, sentiment_comparisons: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            most_positive = max(sentiment_scores.items(), key=lambda x: x[1])[0]
            most_negative = min(sentiment_scores.items(), key=lambda x: x[1])[0]
            ...
            return {
                ...
                "sentiment_range": round(max(sentiment_scores.values()) - min(sentiment_scores.values()), 3),
                "stocks_analyzed": len(valid_sentiments),
                "sentiment_distribution": {
                    "positive": len([s for s in sentiment_scores.values() if s > 0.1]),
                    "negative": len([s for s in sentiment_scores.values() if s < -0.1]),
            ...
            return {"error": f"Comparison creation failed: {str(e)}"}
